...
def print_packages_for_destination_and_price():
    ...
    assert (domain.get_end_date(all_packages[0]) == "01/01/2022")
    assert (domain.get_price(all_packages[0]) == 500)
    assert (domain.get_id(all_packages[1]) == 2)
    assert (domain.get_destination(all_packages[1]) == "Paris")
    assert (domain.get_start_date(all_packages[1]) == "01/01/2022")
    assert (domain.get_end_date(all_packages[1]) == "01/01/2022")
    assert (domain.get_price(all_packages[1]) == 700)
    
    
def print_packages_for_end_date():
    all_packages = []
    op.add_package(all_packages, 1, "07/01/2022", "10/01/2022", "Bologna", 250)
    op.add_package(all_packages, 2, "10/02/2022", "14/02/2022", "Bucharest", 500)
    op.add_package(all_packages, 3, "01/01/2022", "03/01/2022", "Paris", 1500)
    assert op.print_packages_for_end_date(all_packages, "10/01/2022")
    assert (domain.get_id(all_packages[0]) == 1)
    assert (domain.get_destination(all_packages[0]) == "Bologna")
    assert (domain.get_start_date(all_packages[0]) == "07/01/2022")
    assert (domain.get_end_date(all_packages[0]) == "10/01/2022")
    assert (domain.get_price(all_packages[0]) == 250)
    
    
def print_number_of_packages_for_destination():
    all_packages = []
    op.add_package(all_packages, 1, "10/02/2022", "14/02/2022", "Bucharest",  500)
    op.add_package(all_packages, 2, "07/01/2022", "10/01/2022", "Bologna",  250)
    op.add_package(all_packages, 3, "01/01/2022", "02/01/2022", "Paris",  1500)
...
def print_medium_price_for_destination():
    all_packages = []
    op.add_package(all_packages, 1, "10/02/2022", "14/02/2022", "Bucharest", 500)
    op.add_package(all_packages, 2, "07/01/2022", "10/01/2022", "Bucharest", 700)
    op.add_package(all_packages, 3,  "01/01/2022", "02/01/2022", "Paris", 1500)
...
def remove_package_for_other_month():
    all_packages = []
    op.add_package(all_packages, 1, "01/01/2022", "02/01/2022", "Paris", 1500)
    op.add_package(all_packages, 2, "10/10/2022", "14/10/2022", "Bucharest", 500)
    op.remove_package_for_other_month(all_packages, 1)
    assert (domain.get_destination(all_packages[0]) == "Paris")
    assert (domain.get_start_date(all_packages[0]) == "01/01/2022")
